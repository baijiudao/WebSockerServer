...
// 获取当前模块的版本信息
//------------------------------------------------------------------------------
std::string GetSelfProductVersion()
{
  ...
  shared_ptr<CGetFileVersion> pVer = make_shared<CGetFileVersion>();
  pVer->Open(szFileName);

  std::string strProductVer = "[" + to_string(pVer->GetProductVersionMajor()) + "_"
    + to_string(pVer->GetProductVersionMinor()) + "_"
    + to_string(pVer->GetProductVersionBuild()) + "_"
    + to_string(pVer->GetProductVersionQFE()) + "]";
